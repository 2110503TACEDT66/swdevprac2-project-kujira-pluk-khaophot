...
export default async function EditCar(){
    ...
    const update= async (addCarForm:FormData) => {
        ...
        if (cid === null) {
            // Handle the case where 'id' is not found in the form data
            return;
        }
        const cidString: string = cid.toString();
        ...
        try {
            const car:CarItem ={
                ...
                "tel": tel ? String(tel) : "",
                "car": carModel ? String(carModel) : "",
                "dayRate": dayRate ? Number(dayRate) : 0,
                "picture": picture ? String(picture) : "",
                "topSpeed":topSpeed ? Number(topSpeed) : 0,
                "seats": seats ? Number(seats) : 0,
                "color": color ? String(color) : "",
                "fuelType": fuelType ? String(fuelType) : "",
                "cargoCapacity": cargoCapacity ? Number(cargoCapacity) : 0,
                "picArray": [picture1, picture2, picture3].filter(pic => pic !== null).map(pic => String(pic))
            }
            console.log(car)
           await updateCar(session?.user.token,car,cidString)
        } catch (error) {
            console.log(error)
        }
        revalidateTag("cars")
